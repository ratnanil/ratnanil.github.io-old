...
I filed [an issue on sf' github repo](https://github.com/r-spatial/sf/issues/1292) beginning of 2020, but it wasn't until recently (with PROJ 7.0.0) that I was able to **precisely** transform my data to and from EPSG 2056. 
...
I now want to use this grid to visualize how the new coordinate system is different from the old. 
...
markers <- tribble(
  ...
  2682217.000, 1247945.250 , "Zürich"
)
...
# create a "mask" to hide areas outside Switzerland (hacky but fast)
bb_ch <- landesgebiet %>%
  st_bbox() %>%
  st_as_sfc() %>%
  st_buffer(2000) %>%
...
# use cowplot to combine the main plot and the legend
library(cowplot)
ggdraw(p1) +
