...
description: |
  Fedaralistism at its best: How the open data policy is handled across cantons
author:
...
```{r, code = readLines("get_data_geodienste.R"), eval = FALSE, code_folding = "show code for webscraping"}
```


```{r, code_folding = "show code for data preparation"}
...
```{r, layout = "l-page", fig.height=10, code_folding = "show code for creating the plot", fig.cap="All labels are in german for practical reasons. The labels (from left to right): 1) freely availalbe, 2) registration necessary, 3) approval necessary, 4) in the making, 5) no data"}
